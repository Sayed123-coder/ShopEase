...
const SellerNegotiations = () => {
  // ✅ sellerNegotiations use karo — negotiations nahi
  const { sellerNegotiations, loading, getSellerNegotiations, respondToOffer } = useNegotiation();
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="container-custom py-6">
          <h1 className="text-2xl font-bold text-gray-900">Product Negotiations</h1>
          <p className="text-gray-400 text-sm mt-1">
            {sellerNegotiations.length > 0
              ? `${sellerNegotiations.length} negotiation${sellerNegotiations.length > 1 ? 's' : ''}`
              : 'No negotiations yet'}
          </p>
        </div>
      </div>
      ...
      <div className="container-custom py-8 max-w-4xl">
        ...
        {sellerNegotiations.length === 0 ? (
          <div className="text-center py-24">
            ...
            <p className="text-gray-400 text-sm">Buyers will send offers on your products here.</p>
          </div>
        ) : (
          <div className="space-y-4">
            <AnimatePresence>
              {sellerNegotiations.map((n, i) => {
